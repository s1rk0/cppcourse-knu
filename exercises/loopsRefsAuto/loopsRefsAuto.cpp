...
int main() {
   ...
   for (int i = 0; i < 10; ++i) {
      collection[i].resultA = i * 2;     // arbitrary initialization
      collection[i].resultB = i * 2 + 1; // or any other ascending logic
  }
   ...
   int resultA = 0;
   int resultB = 0;
   // Here, we only read from each element, so we use const &.
